...
def get_places_along_route(api_key, route_coords, my_type, search_radius=1000, step_distance=10):
    places = []
    prev_coords = None
    for coords in route_coords:
        if prev_coords is not None:
            dist = distance(prev_coords['lat'], prev_coords['lng'], coords['lat'], coords['lng'])
            if dist >= step_distance:
                places.extend(find_nearest(api_key, coords['lat'], coords['lng'], my_type, search_radius))
                prev_coords = coords
        else:
            prev_coords = coords

    # Final check for the last segment if no place found yet
    if not places and route_coords:
        places.extend(find_nearest(api_key, route_coords[-1]['lat'], route_coords[-1]['lng'], my_type, search_radius))

    return places


api_key = current_app.config['GOOGLE_MAPS_API_KEY']  # Replace with your actual API key
